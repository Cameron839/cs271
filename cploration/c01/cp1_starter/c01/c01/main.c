...
{
    //set var first spot sayings its the largest in the arr
    int biggest = arr[0];
    
    //iterate over the arr
    for (int i = 1; i < n; i++){
        //if spot at i is bigger than biggest
        if (arr[i] > biggest){
            //biggest var is now that spot at i
            biggest = arr[i];
        }
    }
    //return biggest
...
int sum(int arr[], int n)
{
    ...
    //iterate over the arr
    for (int i = 0; i < n; i++){
        //add each number in arr to counter
        total += arr[i];
    }
    //return counter
    return total;
...
double pie(long int n){
    //set result to 0
    double pie_result = 0.0;
    //set numerator to 1;
    double factor = 1.0;
    
    //iterate over pi to number of terms
    for (long int i = 0; i < n; i++, factor = -factor){
        //add neibniz formula to result
        pie_result += factor/(2*i+1);
    }
    //return result
    return 4.0 * pie_result;
}
